...
v0 = I don't even know what I want
"""
...
root_dir = 'D:/models_neonates/ISN/ISN_anesthesia/simulations/'
v = 0
...
AMPA_mods   = np.linspace(0.4,1,4)
GABA_mods   = np.linspace(0.2,4,4)
connectivity = 1
...
for iAMPA, AMPA_mod in enumerate(AMPA_mods):
    for iGABA, GABA_mod in enumerate(GABA_mods):
        ...
        # connect to PYRs
        gEEext = lognormal(0, 1, nPYRS*num_imputs) / 30
        SPYR = Synapses(extInput, PYRs, 'w: 1', on_pre='gea+=w')
        SPYR.connect(p=epsilonPoisson); SPYR.w = gEEext
