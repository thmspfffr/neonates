...
AMPA_mods   = np.linspace(0.1,2,9)
GABA_mods   = np.linspace(0.1,2,9)
connectivity = 1
...
for iAMPA, AMPA_mod in enumerate(AMPA_mods):
    for iGABA, GABA_mod in enumerate(GABA_mods):
        ...
        gEE = normal(1, .1, nPYRS**2) * AMPA_mod
        gEI = normal(1, .1, nPYRS*nINs) * AMPA_mod
        gIE = normal(1, .1, nPYRS*nINs) * GABA_mod
        gII = normal(0.2, .02, nINs**2) * GABA_mod
