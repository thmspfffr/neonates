...
gEE_AMPA = 0.178 * 1 * nS		                   # Weight of recurrent AMPA synapses between excitatory neurons
#gEE_AMPA_cor = 0.187 * nS		                   # Weight of intra-network AMPA synapses between excitatory neurons
gEE_AMPA_ext = 0.234 * 1 * nS		               # Weight of external AMPA synapses between excitatory neurons
gEI_AMPA = 0.254 * 1 * nS                         # Weight of excitatory to inhibitory synapses (AMPA)
gIE_GABA = 2.01 * 1 * nS                          # Weight of inhibitory to excitatory synapses (GABA)
gII_GABA = 2.7 * 1 * nS                           # Weight of inhibitory to inhibitory synapses (GABA)
...
for iAMPA, AMPA_mod in enumerate(AMPA_mods):
    for iGABA, GABA_mod in enumerate(GABA_mods):
        ...
        for itr in range(0,ntr):
            ...
            fn = os.path.expanduser(root_dir + '/elife_model_iampa%d_gaba%d_tr%d_v%d_processing.txt') % (iAMPA, iGABA, itr, v)
            if os.path.isfile(fn)==False:
                call(['touch', fn])
            else:
                continue

            print('working on AMPA ' + str(iAMPA) + ' GABA ' + str(iGABA) + ' TRIAL ' + str(itr))
            ...
            savemat(root_dir + 'elife_model_v' + str(v) + '_GABA' + str(iGABA) + 
                '_AMPA' + str(iAMPA) + '_tr' + str(itr) + '.mat', dict2save)    
